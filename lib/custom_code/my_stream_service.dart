  ...
  void startListening(Stream<RangingResult> myStream) {
    ...
    _streamRanging ??= myStream.listen((result) async {
      ...
      if (_lastScanProcessedAt != null &&
          now.difference(_lastScanProcessedAt!).inMilliseconds < 200) {
        //ทำงานทุก 200 ms เท่านั้น
        return;
      }
          ...
          if (matchingEvents.length > 0) {
            ...
            await Future.wait(
              matchingEvents.map((event) async {
                ...
                final BoothDataModelStructNew firstBooth =
                    event.boothList.first;
...
/*
    ...
    // หากลงทะเบียนแล้ว ให้ Reset flag เพื่อให้ Noti บูททำงานได้ทันที
 ...
 */
    ...
    if (filteredUserActivity.isNotEmpty) {
    ...
    final logId = await logProximityAndConcurrentUsers(
      ...
      notiData: {
        ...
        'booth_id': booth.boothId,
        'title':
            'ขณะนี้คุณอยู่ใกล้บูท ${booth.boothName} (กิจกรรม ${event.eventName})',
    ...
    //ยังไม่เคยเคยส่งnotiชวนเล่นกิจกรรม event_idนี้
      ...
      await notiRef.add({
        ...
        'booth_id': '${booth.boothId}',
        'title':
            'ขณะนี้คุณได้อยู่ใกล้บูท${booth.boothName} (กิจกรรม ${event.eventName})',
        'body': '${booth.description}',
        'sent_at': FieldValue.serverTimestamp(),
        'event_id': int.parse('${event.eventId}'),
        'send_count': 1,
  ...
  /// niuemon 2025-10-01 ฟังก์ชันสำหรับบันทึกการตรวจจับผู้ใช้ใกล้บูท
  ...
  Future<String?> logProximityAndConcurrentUsers(
    ...
    required bool isStable, // อยู่ใกล้บูทต่อเนื่องครบเวลา
    required double? boothThreshold, // threshold เฉพาะของบูท
  ...
  //     //   print("⏳ User $uid เคยเข้าบูท $boothId แล้ว — ไม่บันทึกซ้ำ");
  //     // } else {
  ...
  //       'stable_status': isStable ? 'stable' : 'unstable',
  //
  //       // ✅ ระยะ threshold ของบูทนี้ (ดึงจาก config หรือ booth.notificationDistance)
